...
app.post('/api/cars', authenticateToken, async (req, res) => {
  try {
    ...
    let query = `INSERT INTO cars (brand, model, year, price, mileage, body_type_id, description, status`;
    let values = [brand, model, year, price, mileage || 0, body_type || 1, description || '', status || 'available'];
    ...
    if (transmission_type != null && transmission_type != '') {
      query += `, transmission_type_id`;
      values.push(transmission_type);
    }
...
app.put('/api/cars/:id', authenticateToken, async (req, res) => {
  try {
    ...
    let setClause = `brand = $1, model = $2, year = $3, price = $4, mileage = $5, body_type_id = $6, description = $7, status = $8`;
    let values = [brand, model, year, price, mileage, body_type, description, status];
    let paramIndex = 9;
    
    // Add optional fields only if they have values
    if (engine_volume != null) {
      setClause += `, engine_volume = ${paramIndex}`;
      values.push(engine_volume);
      paramIndex++;
    }
    if (fuel_type != null && fuel_type != '') {
      setClause += `, fuel_type_id = ${paramIndex}`;
      values.push(fuel_type);
      paramIndex++;
    }
    if (power != null) {
      setClause += `, power = ${paramIndex}`;
      values.push(power);
      paramIndex++;
    }
    if (transmission_type != null && transmission_type != '') {
      setClause += `, transmission_type_id = ${paramIndex}`;
      values.push(transmission_type);
      paramIndex++;
    }
    if (drive_type != null && drive_type != '') {
      setClause += `, drive_type_id = ${paramIndex}`;
      values.push(drive_type);
      paramIndex++;
    }
    
    values.push(carId); // Add carId as the last parameter
    
    const result = await pool.query(
      `UPDATE cars SET ${setClause} WHERE id = ${paramIndex} RETURNING *`,
      values
    );
