...
app.post('/api/cars', authenticateToken, async (req, res) => {
  try {
    ...
    if (fuel_type != null && fuel_type != '') {
      query += `, fuel_type`;
      values.push(fuel_type); // Direct string value like price
    }
    if (power != null) {
      query += `, power`;
      values.push(power);
    ...
    if (transmission_type != null && transmission_type != '') {
      ...
      values.push(transmission_type); // Direct string value like price
    }
    if (drive_type != null && drive_type != '') {
      query += `, drive_type`;
      values.push(drive_type); // Direct string value like price
    }
...
app.put('/api/cars/:id', authenticateToken, async (req, res) => {
  try {
    ...
    let setClause = `brand = $1, model = $2, year = $3, price = $4, mileage = $5, body_type = $6, description = $7, status = $8`;
    ...
    if (fuel_type != null && fuel_type != '') {
      setClause += `, fuel_type = ${paramIndex}`;
      values.push(fuel_type); // Direct string value like price
      paramIndex++;
    }
    if (power != null) {
    ...
    if (transmission_type != null && transmission_type != '') {
      setClause += `, transmission_type = ${paramIndex}`;
      values.push(transmission_type); // Direct string value like price
      paramIndex++;
    }
    if (drive_type != null && drive_type != '') {
      setClause += `, drive_type = ${paramIndex}`;
      values.push(drive_type); // Direct string value like price
      paramIndex++;
    }
